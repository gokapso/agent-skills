...
- For template creation/sending, use the Meta proxy endpoints (see `whatsapp-templates` skill).
- For WhatsApp Flows, use Platform API flow endpoints (see `whatsapp-flows` skill).
- For workflow graph edits, use workflow definition endpoints (see `workflow-builder` skill).
