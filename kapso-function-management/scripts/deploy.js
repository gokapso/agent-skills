...
async function main() {
  ...
  if (hasHelpFlag(argv)) {
    console.log(
      JSON.stringify(
        {
          ok: true,
          usage: 'node /agent-skills/kapso-function-management/scripts/deploy.js --function-id <id>',
  ...
  try {
    ...
    const data = await kapsoRequest(config, `/functions/${encodeURIComponent(functionId)}/deploy`, {
      method: 'POST',
      body: JSON.stringify({})
    });
