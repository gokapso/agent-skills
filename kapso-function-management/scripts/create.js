...
async function main() {
      ...
      JSON.stringify(
        {
          ok: true,
          usage:
            'node /agent-skills/kapso-function-management/scripts/create.js --name <name> (--code <js> | --code-file <path>) [--description <text>]',
  ...
  try {
    ...
    const data = await kapsoRequest(config, '/functions', {
      method: 'POST',
