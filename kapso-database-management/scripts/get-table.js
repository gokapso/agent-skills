...
const { hasHelpFlag, parseFlags, requireFlag, parseNumber } = require('./lib/args');
...
async function main() {
  ...
  if (hasHelpFlag(argv)) {
    console.log(
      JSON.stringify(
        {
          ok: true,
          usage: 'node /agent-skills/kapso-database-management/scripts/get-table.js --table <name> [--limit <n>]',
  ...
  try {
    ...
    const data = await kapsoRequest(config, `/database_tables/${encodeURIComponent(table)}${query}`);
